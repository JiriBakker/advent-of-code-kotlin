...
private class Cell private constructor (override val x: Int, override val y: Int, val isWall: Boolean, occupiedBy: Fighter?) :
    IPositioned {
                ...
                for (x in 0 until width) {
                    val cell = when (inputLines[y][x]) {
                        'E' -> {
                            ...
                            fighters.add(elf)
                            Cell.Companion.withFighter(x, y, elf)
                        }
                        'G' -> {
                            val goblin = Fighter.goblin(x, y)
                            fighters.add(goblin)
                            Cell.Companion.withFighter(x, y, goblin)
                        }
                        '.' -> Cell.Companion.empty(x, y)
                        else -> Cell.Companion.wall(x, y)
                    }
                    grid[y].add(cell)
                }
            }
